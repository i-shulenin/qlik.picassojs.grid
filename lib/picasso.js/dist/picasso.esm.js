...
function rotatePoint$1(point, angle) {
  ...
  var Path2D = /*#__PURE__*/function () {
    function Path2D(path) {
      ...
      if (path && path instanceof Path2D) {
        ...
        (_this$segments = this.segments).push.apply(_this$segments, _toConsumableArray$1(path.segments));
      } else if (path) {
        this.segments = parsePath(path);
      }
    }

    _createClass$1(Path2D, [{
      key: "addPath",
      value: function addPath(path) {
        if (path && path instanceof Path2D) {
          var _this$segments2;

          (_this$segments2 = this.segments).push.apply(_this$segments2, _toConsumableArray$1(path.segments));
        }
      }
    }, {
      key: "moveTo",
    ...
    for (var i = 0; i < segments.length; ++i) {
      ...
      switch (pathType) {
        ...
        case 'A':
          ...
          rotatePoint$1(midPoint, -angle); // radius correction
          ...
          rotatePoint$1(centerPoint, angle);
...
function pointsToRect(points) {
  var _getMinMax = getMinMax$1(points),
...
function getShapeType(shape) {
  ...
  if (Array.isArray(vertices)) {
    return vertices.every(function (item) {
...
function datasets(dataSources, _ref) {
  ...
  if (!Array.isArray(dataSources)) {
    ...
    sets.push.apply(sets, _toConsumableArray(dataSources));
  }
...
function formatter(pattern) {
  ...
  format.locale = function localeFn() {
    locale = formatLocale.apply(void 0, arguments);
    d3format = locale.format(pattern);
    return this;
  };
  /**
    ...
    if ('collection' in dataConfig) {
      ...
      if (dataConfig.groupBy || dataConfig.mapTo) {
        ...
        extractionsConfigs.forEach(function (cfg) {
          ...
          (_extracted$items = extracted.items).push.apply(_extracted$items, _toConsumableArray(s.extract(cfg)));
        ...
        if (dataConfig.amend && Array.isArray(dataConfig.amend)) {
          ...
          (_extracted$items2 = extracted.items).push.apply(_extracted$items2, _toConsumableArray(dataConfig.amend));
        }
...
function create$p(config, d, opts) {
  var extractor = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : extract$1;
  var collections = {};
...
function create$o(options, data, deps) {
  var extractor = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : extract$1;
...
function collection$1(formattersConfig, data, deps) {
  var fn = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : create$o;
  var formatters = {};
  return {
...
function appendMinorTicks(majorTicks, minorCount, scale) {
  ...
  for (var i = 0; i < majorTicks.length; i++) {
    ...
    if (i === 0 && start !== scale.start()) {
      ...
      ticks.push.apply(ticks, _toConsumableArray(minorTicksGenerator(minorCount, start, end)));
      start -= end - start;
      end = majorTicks[i];
      ticks.push.apply(ticks, _toConsumableArray(minorTicksGenerator(minorCount, start, end)));
...
function getMinMax(settings, fields) {
  ...
  if (fields && fields[0]) {
    ...
    fieldMin = minValues.length ? Math.min.apply(Math, _toConsumableArray(minValues)) : Number.NaN;
    fieldMax = maxValues.length ? Math.max.apply(Math, _toConsumableArray(maxValues)) : Number.NaN;
    ...
    if (Array.isArray(settings.include)) {
      ...
      fieldMin = Math.min.apply(Math, _toConsumableArray(i).concat([fieldMin]));
      fieldMax = Math.max.apply(Math, _toConsumableArray(i).concat([fieldMax]));
    }
  }
...
function scaleLinear() {
  ...
  var stgns = resolveSettings$2(settings, DEFAULT_SETTINGS$8, ctx);
  ...
  var _getMinMax = getMinMax(stgns, data ? data.fields : []),
...
function scaleBand() {
  ...
  var stgns = resolveSettings$2(settings, DEFAULT_SETTINGS$7, ctx);
...
function flattenTree(rootNode, settings, ctx) {
  ...
  rootNode.eachAfter(function (node) {
    if (node.depth > 0) {
      ...
      var value = valueFn(extend$1({
        datum: node.data
      }, ctx));
      var label = labelFn(extend$1({
        datum: node.data
      }, ctx));
...
function minmax(settings, arr) {
  ...
  if (arr && arr.length) {
    if (!definedMin) {
      ...
      min = arrMin.length ? Math.min.apply(Math, _toConsumableArray(arrMin)) : min;
    }

    if (!definedMax) {
      var arrMax = arr.map(maxAccessor).filter(isNumber);
      max = arrMax.length ? Math.max.apply(Math, _toConsumableArray(arrMax)) : max;
    }
  }
...
function scaleSequentialColor() {
  ...
  var stgns = resolveSettings$2(settings, DEFAULT_SETTINGS$6, {
    data: data,
    resources: resources
  });
...
function generateRange(domain, colors, min, max) {
  ...
  var values = [min].concat(_toConsumableArray(getBreaks(domain)), [max]);
...
function scaleThresholdColor() {
  ...
  var stgns = resolveSettings$2(settings, DEFAULT_SETTINGS$5, {
    data: data,
    resources: resources
  });
  var isDomain = Array.isArray(stgns.domain) && stgns.domain.length;
...
function ordinal() {
  ...
  var stgns = resolveSettings$2(settings, DEFAULT_SETTINGS$4, ctx);
  var valueFn = typeof settings.value === 'function' ? settings.value : function (d) {
    return d.datum.value;
  };
  var labelFn = typeof settings.label === 'function' ? settings.label : function (d) {
...
function scaleCategorical() {
  ...
  var data = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
  var resources = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
  var s = ordinal(settings, data, resources);
  var theme = resources.theme;
  var stgns = resolveSettings$2(settings, DEFAULT_SETTINGS$3, {
    data: data,
    resources: resources
  });
...
function create$n(options, d, deps) {
  ...
  var data = extract$1(dataSourceConfig, d, deps);
...
function collection(scalesConfig, data, deps) {
  var fn = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : create$n;
  var scales = {};
  return {
    get: function get(def) {
      var key;
...
function brush() {
  ...
  fn.start = function () {
    if (!activated) {
      ...
      fn.emit.apply(fn, ['start'].concat(args));
      links.start();
    }
  };
  /**
  ...
  fn.end = function () {
    ...
    fn.emit.apply(fn, ['end'].concat(args));
    links.end();
  };
  /**
...
function tween(_ref, _ref2, config) {
  ...
  var tweener = {
    start: function start() {
      ...
      Object.keys(ids).forEach(function (key) {
        if (ids[key]) {
          ...
          exited.ips.push(interpolateObject(ids[key], extend$1({}, ids[key], {
            r: 0.0001,
            opacity: 0
          })));
        }
      ...
      if (entered.ips.length) {
        stages.push({
          ...
          duration: 1200,
          tweens: entered.ips,
          nodes: _toConsumableArray(updated.nodes)
        });
      } // console.log(stages);
    ...
    tick: function tick() {
      ...
      currentNodes.push.apply(currentNodes, _toConsumableArray(tweenedNodes));
      currentNodes.push.apply(currentNodes, _toConsumableArray(currentStage.nodes)); // currentNodes.push(...staticNodes);
...
function normalizeSettings(settings, defaults, chart) {
  var composition = extend$1({}, settings);
  var defs = extend$1({}, defaults);
...
function reduceToLeafNodes() {
  ...
  return nodes.reduce(function (ary, node) {
    if (Array.isArray(node.children)) {
      ary.push.apply(ary, _toConsumableArray(reduceToLeafNodes(node.children)));
      return ary;
    }
...
function styler(obj, _ref) {
  ...
  var update = function update() {
    ...
    var evaluatedDataProps = typeof dataProps === 'function' ? dataProps({
      brush: brusher
    }) : dataProps;
    ...
    var _loop = function _loop(i) {
      ...
      var isActive = brusher.containsMappedData(nodeData, evaluatedDataProps, mode);
      ...
      if (changed || globalActivation) {
        var original = extend$1({}, nodes[i], nodes[i].__style);
        styleProps.forEach(function (s) {
  ...
  var onStart = function onStart() {
    ...
    var suppressRender = opts.suppressRender;
    var nodes = getNodes();
    var len = nodes.length;
...
function componentFactory(definition) {
  ...
  var _settings = extend$1(true, {}, defaultSettings, config);
  ...
  var instanceContext = extend$1({}, config); // Create a callback that calls lifecycle functions in the definition and config (if they exist).
  ...
  var _dockConfig = create$l(createDockDefinition(_settings, preferredSize, _chart.logger()), dockConfigCallbackContext);
  ...
  fn.set = function () {
    ...
    if (opts.settings) {
      ...
      _settings = extend$1(true, {}, defaultSettings, opts.settings);
      _dockConfig = create$l(createDockDefinition(_settings, preferredSize, _chart.logger()), dockConfigCallbackContext);
    }
    ...
    if (_settings.data) {
      _data = extract$1(_settings.data, {
  ...
  fn.render = function () {
    var nodes = brushArgs.nodes = render.call.apply(render, [definitionContext].concat(_toConsumableArray(getRenderArgs())));
    rend.render(nodes);
    currentNodes = nodes;
  };
  ...
  fn.update = function () {
    ...
    var nodes = brushArgs.nodes = render.call.apply(render, [definitionContext].concat(_toConsumableArray(getRenderArgs()))); // Reset brush stylers and triggers
  ...
  prepareContext(definitionContext, definition, {
    ...
    symbol: function symbol() {
      return create$m(_registries.symbol);
    }
  });
  /**
   * Component instance
...
function closestPointToLine(start, end, p) {
  ...
  var pointOnLine = add(projectOnto(startToPoint, startToEnd), start);
  return pointOnLine;
}
function isPointOnLine(start, end, p) {
  return distance(start, p) + distance(end, p) - distance(start, end) < EPSILON$2;
}
/**
...
function testPolygonPolygonSubCase(polygon1, polygon2) {
  ...
  for (var i = 0, len = polygon2.edges.length; i < len; i++) {
...
function testGeoPolygonPolygonCase2(geopolygon, polygon) {
  var intersects = false;
  var numPolygons = geopolygon.numPolygons,
      polygons = geopolygon.polygons;

  for (var n = 0; n < numPolygons; n++) {
...
function testGeoPolygonGeoPolygonSubCase(geopolygon1, geopolygon2) {
  var intersects = false;
  var numPolygons = geopolygon2.numPolygons,
      polygons = geopolygon2.polygons;
...
function testPolygonPolygon(polygon1, polygon2) {
  var rect1 = polygon1.boundingRect();
  var rect2 = polygon2.boundingRect();
...
function testGeoPolygonPolygon(geopolygon, polygon) {
  var rect1 = geopolygon.boundingRect();
  var rect2 = polygon.boundingRect();
...
function testGeoPolygonGeoPolygon(geopolygon1, geopolygon2) {
  var rect1 = geopolygon1.boundingRect();
  var rect2 = geopolygon2.boundingRect();
...
function testGeoPolygonLine(geopolygon, line) {
  var _lineToPoints13 = lineToPoints(line),
      _lineToPoints14 = _slicedToArray(_lineToPoints13, 2),
      p1 = _lineToPoints14[0],
      p2 = _lineToPoints14[1];
...
function testGeoPolygonRect(geopolygon, rect) {
  var _rectToPoints3 = rectToPoints(rect),
      _rectToPoints4 = _slicedToArray(_rectToPoints3, 4),
      p1 = _rectToPoints4[0],
      p2 = _rectToPoints4[1],
      p3 = _rectToPoints4[2],
      p4 = _rectToPoints4[3];
...
function res(style, references, path) {
  ...
  var computed = style;
  var refs = extend$1(true, {}, references, style);
  var s = {};
...
function themeFn() {
  ...
  var theme = {
    ...
    style: function style(s) {
      return resolve$1(s, internalStyle);
    },
...
function reduceSingleLayoutRect(layoutRect, reducedRect, edgeBleed, c, settings) {
  var newReduceRect = extend$1({}, reducedRect);
  var newEdgeBleed = extend$1({}, edgeBleed);
...
function reduceLayoutRect(_ref) {
  ...
  visible.length = 0;
  visible.push.apply(visible, _toConsumableArray(filteredUnsortedComps));
...
function boundingBox(rects) {
  ...
  var points = (_ref2 = []).concat.apply(_ref2, _toConsumableArray(rects.map(rectToPoints)));
...
function dockLayout(initialSettings) {
  var settings = resolveSettings$1(initialSettings);
  var docker = {};
  ...
  docker.layout = function layout(rect) {
    ...
    var ordered = positionComponents({
      visible: visible,
      layoutRect: logicalContainerRect,
    ...
    return {
      ...
      ordered: ordered.map(function (h) {
        return h.comp;
      })
    };
  };
...
var findComponentByKeyInList = function findComponentByKeyInList(list, key) {
...
var wrapChildren = function wrapChildren(children) {
  return children === null || children === void 0 ? void 0 : children.map(function (c) {
...
var hideAll = function hideAll(rect, components) {
  return {
    visible: [],
    hidden: components,
    ordered: []
...
var customLayout = function customLayout(fn) {
  return function (rect, components) {
    var _fn;

    var vcomponents = components.map(function (c, i) {
      var dockConfig = c.instance.dockConfig();
      return {
        index: i,
        key: c.key,
    ...
    var mapBack = function mapBack(c) {
      return components[c.index];
    };

    var _ref = (_fn = fn(rect, vcomponents)) !== null && _fn !== void 0 ? _fn : {},
        _ref$visible = _ref.visible,
        visible = _ref$visible === void 0 ? vcomponents : _ref$visible,
        _ref$hidden = _ref.hidden,
        hidden = _ref$hidden === void 0 ? [] : _ref$hidden,
        _ref$ordered = _ref.ordered,
        ordered = _ref$ordered === void 0 ? visible : _ref$ordered;
...
var normalLayout = function normalLayout(layoutSettings) {
  var dockLayout$1 = dockLayout(layoutSettings);
  return customLayout(function (rect, vcomponents) {
    return dockLayout$1.layout(rect, vcomponents);
  });
};

var getLayoutFn = function getLayoutFn(strategy) {
  return typeof strategy === 'function' ? customLayout(strategy) : normalLayout(strategy);
};

function collectionFn(_ref2) {
  var createComponent = _ref2.createComponent;
  var instance = {};
  var allComponents = [];
  var topComponents = [];

  var createComp = function createComp(compSettings) {
    var component = createComponent(compSettings);

    if (component) {
      allComponents.push(component);

      if (compSettings.components) {
        component.children = compSettings.components.map(function (childSettings) {
          return createComp(childSettings);
        }).filter(function (c) {
          return !!c;
        });
      }
  ...
  var removeFromAllComponents = function removeFromAllComponents(component) {
    var index = allComponents.indexOf(component);
  ...
  var removeDeleted = function removeDeleted(compList, settingsList) {
    var _loop = function _loop(i) {
      var currComp = compList[i]; // TODO warn when there is no key

      var currSettings = settingsList.find(function (c) {
  ...
  var addAndUpdate = function addAndUpdate(_ref3) {
    var compList = _ref3.compList,
        data = _ref3.data,
        excludeFromUpdate = _ref3.excludeFromUpdate,
        formatters = _ref3.formatters,
        scales = _ref3.scales,
        settingsList = _ref3.settingsList;
    // Let the "components" array determine order of components
    return settingsList.map(function (comp) {
      var component = findComponentByKeyInList(compList, comp.key); // Component should not be updated
  ...
  var recLayout = function recLayout(_ref4) {
    var components = _ref4.components,
        hidden = _ref4.hidden,
        layoutFn = _ref4.layoutFn,
        ordered = _ref4.ordered,
        rect = _ref4.rect,
        visible = _ref4.visible;

    var _layoutFn = layoutFn(rect, components),
        v = _layoutFn.visible,
        h = _layoutFn.hidden,
        o = _layoutFn.ordered;

    visible.push.apply(visible, _toConsumableArray(v));
    hidden.push.apply(hidden, _toConsumableArray(h));
    ordered.push.apply(ordered, _toConsumableArray(o));
    v.forEach(function (c) {
      if (c.children) {
        var _c$settings$strategy, _c$settings;

        var lFn = getLayoutFn((_c$settings$strategy = (_c$settings = c.settings) === null || _c$settings === void 0 ? void 0 : _c$settings.strategy) !== null && _c$settings$strategy !== void 0 ? _c$settings$strategy : {});
        recLayout({
  ...
  instance.findComponentByKey = function (key) {
    return findComponentByKeyInList(allComponents, key);
  };

  instance.forEach = function (fn) {
    allComponents.forEach(fn);
  };

  instance.layout = function (_ref5) {
    var layoutSettings = _ref5.layoutSettings,
        rect = _ref5.rect;
    var visible = [];
    var hidden = [];
    var ordered = [];
    var layoutFn = getLayoutFn(layoutSettings);
    recLayout({
  ...
  instance.set = function (_ref6) {
    var components = _ref6.components;
    topComponents = components.map(function (compSettings) {
      return createComp(compSettings);
    }).filter(function (c) {
      return !!c;
    });
  };

  instance.update = function (_ref7) {
    var components = _ref7.components,
        data = _ref7.data,
        excludeFromUpdate = _ref7.excludeFromUpdate,
        formatters = _ref7.formatters,
        scales = _ref7.scales;
    // remove deleted
...
function addComponentDelta(shape, containerBounds, componentBounds) {
  ...
  var deltaShape = extend$1(true, {}, shape);
  ...
  switch (type) {
    ...
    case 'geopolygon':
      ...
      for (var n = 0; n < deltaShape.vertices.length; n++) {
        var vertices = deltaShape.vertices[n];

        for (var _i = 0, _num = vertices.length; _i < _num; _i++) {
          var _v = vertices[_i];
          _v.x += dx;
          _v.y += dy;
        }
...
function chartFn(definition, context) {
  ...
  var instance = extend$1({}, definition);
  ...
  var createComponent = function createComponent(compSettings) {
  ...
  var set = function set(_data, _settings) {
    ...
    dataCollection = create$p(_settings.collections, {
      dataset: dataset
    }, {
      logger: logger
    });
  ...
  instance.update = function () {
    ...
    componentsC.forEach(function (comp) {
      if (comp.updateWith) {
    ...
    if (partialData) {
      ...
      var _layout2 = layout(),
          ...
          ordered = _layout2.ordered; // Relayout
  ...
  instance.getAffectedShapes = function (ctx) {
    ...
    var shapes = [];
    visibleComponents.filter(function (comp) {
      return key === undefined || key === null || comp.key === key;
    }).forEach(function (comp) {
      shapes.push.apply(shapes, _toConsumableArray(comp.instance.getBrushedShapes(ctx, mode, props)));
    });
    return shapes;
  };
  /**
  ...
  instance.findShapes = function (selector) {
    var shapes = [];
    visibleComponents.forEach(function (c) {
      shapes.push.apply(shapes, _toConsumableArray(c.instance.findShapes(selector)));
    });
    return shapes;
  };
  /**
  ...
  instance.shapesAt = function (shape) {
    ...
    for (var i = comps.length - 1; i >= 0; i--) {
      ...
      result.push.apply(result, _toConsumableArray(shapes));
  ...
  instance.component = function (key) {
    var component = componentsC.findComponentByKey(key);
...
function collect(trackedItems, _ref2) {
  ...
  dataItems.push.apply(dataItems, _toConsumableArray(trackedItems.map(function (t) {
  ...
  cfgs.forEach(function (cfg) {
    if (typeof cfg.field !== 'undefined') {
      (function () {
        ...
        if (track$1) {
          dataItems.push.apply(dataItems, _toConsumableArray(collect(trackedItems, {
            main: main,
            propsArr: propsArr,
            props: props
          })));
...
function createFields(_ref) {
  ...
  for (var _c2 = 0; _c2 < flds.length; _c2++) {
    ...
    var min = isMeasure ? Math.min.apply(Math, _toConsumableArray(numericValues)) : NaN;
    var max = isMeasure ? Math.max.apply(Math, _toConsumableArray(numericValues)) : NaN;
...
function ds() {
  ...
  var dataset = {
     ...
     */
    extract: function extract$1(cfg) {
      return extract(cfg, dataset);
    },
...
function oob(_ref) {
  ...
  return symbol(extend$1({}, item.oob, (_extend = {}, _defineProperty(_extend, x, boxCenter * calcwidth), _defineProperty(_extend, y, Math.max(item.oob.size / 2, Math.min(value * calcheight, calcheight - item.oob.size / 2))), _defineProperty(_extend, "startAngle", startAngle), _extend)));
}
/**
...
function box$1(_ref2) {
  ...
  return extend$1({}, item.box, (_extend2 = {
    type: 'rect'
...
function verticalLine(_ref3) {
  ...
  return extend$1({}, item.line, (_extend3 = {
    type: 'line'
...
function complexResolver(_ref) {
  ...
  var defaults = extend$1(true, {}, defaultSettings || {}, style || {});
  var scaled = {
...
var dataKeys = Object.keys(DEFAULT_DATA_SETTINGS$2);
var component$3 = {
  ...
  render: function render(_ref) {
    ...
    keys.forEach(function (key) {
      return defaultSettings[key] = DEFAULT_DATA_SETTINGS$2[key];
    });
...
function getType(s) {
  var type = DEFAULT_DATA_SETTINGS$1.shape;
  var props = {};

  if (_typeof(s.shape) === 'object' && typeof s.shape.type === 'string') {
    type = s.shape.type;
    props = s.shape;
...
function createDisplayPoints(dataPoints, _ref, pointSize, shapeFn) {
  ...
  return dataPoints.filter(function (p) {
    return p.show !== false && !isNaN(p.x + p.y);
  }).map(function (p) {
    var s = p;
    var size = PX_RX.test(p.size) ? parseInt(p.size, 10) : pointSize.min + s.size * (pointSize.max - pointSize.min);
    ...
    var _getType = getType(s),
        _getType2 = _slicedToArray(_getType, 2),
        type = _getType2[0],
        typeProps = _getType2[1];

    var shapeSpec = _objectSpread2(_objectSpread2({}, typeProps), {}, {
      type: type,
      label: p.label,
      x: p.x * width,
      y: p.y * height,
...
var component$2 = {
  ...
  render: function render(_ref2) {
    ...
    var _this$rect = this.rect,
        width = _this$rect.width,
        height = _this$rect.height;
    var limits = extend$1({}, SIZE_LIMITS, this.settings.settings.sizeLimits);
...
function pointMarker(picasso) {
  picasso.component(type$4, component$2);
...
var pieComponent = {
  ...
  render: function render(_ref2) {
    ...
    var pieGen = pie$1().sortValues(null);
...
var Transposer = /*#__PURE__*/function () {
  ...
  _createClass(Transposer, [{
     ...
     */
    function transposeCoordinate(key, coordinate, flipXY) {
...
function createLineWithLabel(_ref) {
  ...
  var label = false;
  var value = false;
  var style = extend$1(true, {}, settings.style.line, p.line || {}); // Use the transposer to handle actual positioning
  ...
  if (p.label) {
    var item = extend$1(true, refLabelDefaultSettings(), settings.style.label || {}, {
      fill: style.stroke
    }, p.label);
    var formatter;
    var measuredValue = {
...
function filterUndefinedValue(line) {
  var value = typeof line.value === 'function' ? line.value() : line.value;
  ...
  render: function render() {
    ...
    this.lines.x = this.lines.x.filter(filterUndefinedValue).map(function (line) {
      ...
      if (line.scale) {
        ...
        var position = getPosition(scale, line.value);
    ...
    [].concat(_toConsumableArray(this.lines.x), _toConsumableArray(this.lines.y)).forEach(function (p) {
...
function isTiltedLabelOverlapping(_ref4) {
  ...
  var size = Math.min(rect.outer.width - bleedSize, rect.inner.width);
...
function getSize(_ref7) {
  ...
  if (settings.labels.show) {
    ...
    var distance = horizontal ? rect.inner.width : rect.inner.height;
    ...
    if (!settings.labels.filterOverlapping && state.labels.activeMode !== 'tilted' && isToLarge({
      ...
      var toLargeSize = Math.max(rect.outer.width, rect.outer.height); // used to hide the axis
    ...
    var textSize = Math.max.apply(Math, _toConsumableArray(labelSizes).concat([0]));
    size += textSize;
    ...
    if (state.labels.activeMode === 'tilted') {
      ...
      var bleedSize = Math.min(settings.labels.maxEdgeBleed, Math.max.apply(Math, _toConsumableArray(tickMeasures.map(labelWidth).map(adjustByPosition)).concat([0]))) + settings.paddingEnd;
      ...
      if (!settings.labels.filterOverlapping && isTiltedLabelOverlapping({
        ...
        var _toLargeSize = Math.max(rect.outer.width, rect.outer.height); // used to hide the axis
...
var getLeftEdgeWidth = function getLeftEdgeWidth(_ref) {
  ...
  var minWidth = tick.position === 0 ? innerRect.width / 2 - PADDING$1 : 0;
...
var getRightEdgeWidth = function getRightEdgeWidth(_ref2) {
  ...
  var minWidth = tick.position === 1 ? innerRect.width / 2 - PADDING$1 : 0;
...
function getHorizontalWidth(_ref3) {
  ...
  var prevWidth = prev ? tickDistance(innerRect, tick, prev) / 2 - PADDING$1 : Infinity;
  var nextWidth = next ? tickDistance(innerRect, tick, next) / 2 - PADDING$1 : Infinity;
...
function tickBuilder(ticks, buildOpts) {
  return ticks.map(function (tick) {
    return buildNode(tick, buildOpts);
  });
}
...
function labelBuilder(ticks, buildOpts, resolveTickOpts) {
  return ticks.map(function (tick, idx) {
    ...
    var label = buildNode$1(tick, buildOpts);
...
function nodeBuilder(isDiscrete) {
  ...
  function build(_ref5) {
    ...
    if (settings.minorTicks && settings.minorTicks.show && minor.length > 0) {
      ...
      nodes.push.apply(nodes, _toConsumableArray(tickBuilder(minor, buildOpts)));
    }

    if (majorTickNodes) {
      nodes.push.apply(nodes, _toConsumableArray(majorTickNodes));
    }
...
function resolveLocalSettings(_ref2) {
  var state = _ref2.state,
      style = _ref2.style,
      settings = _ref2.settings;
  var defaultStgns = extend$1(true, {}, state.isDiscrete ? DEFAULT_DISCRETE_SETTINGS : DEFAULT_CONTINUOUS_SETTINGS, style);
  var localStgns = extend$1(true, {}, defaultStgns, settings.settings);
  ...
  var paddingStart = localStgns.paddingStart,
      paddingEnd = localStgns.paddingEnd;
...
var axisComponent = {
  ...
  resize: function resize(opts) {
    var inner = opts.inner,
        outer = opts.outer;
    var extendedInner = extend$1({}, inner, alignTransform({
  ...
  render: function render() {
    ...
    var nodes = [];
    nodes.push.apply(nodes, _toConsumableArray(this.state.concreteNodeBuilder.build({
      settings: this.state.settings,
      scale: this.state.pxScale,
...
function axis(picasso) {
  picasso.component(type$3, axisComponent);
}
  ...
  if (scaleData && scaleData.fields) {
    scaleData.fields.forEach(function (field) {
      if (state.fauxBrushInstance) {
        var ordRS = ranges$2(state, state.fauxBrushInstance);
...
function rangesOverlap(r1, r2) {
  return Math.min.apply(Math, _toConsumableArray(r1)) <= Math.max.apply(Math, _toConsumableArray(r2)) && Math.max.apply(Math, _toConsumableArray(r1)) >= Math.min.apply(Math, _toConsumableArray(r2));
}
...
function resolveNodeBounds(targetNodes) {
  var points = targetNodes.reduce(function (ary, node) {
    ary.push.apply(ary, _toConsumableArray(rectToPoints(node.bounds)));
    return ary;
  }, []);
  return pointsToRect(points);
}
...
function setRanges(state) {
  ...
  rs.forEach(function (range) {
    shapes.push.apply(shapes, _toConsumableArray(shapesFromRange(state, range)));
  });
  brushFromShape(state, shapes);
}

function getBubbleLabel(state, value, range) {
  var min = Math.min.apply(Math, _toConsumableArray(range));
  var max = Math.max.apply(Math, _toConsumableArray(range));
  var shapeAt = {
...
function shapes(state) {
  ...
  return ranges(state).map(function (range) {
...
function render$7(state, renderer) {
...
var brushLassoComponent = {
  ...
  move: function move(e) {
    ...
    render$7(this.state, this.renderer);
...
var PADDING = 4; // const DOUBLE_PADDING = PADDING * 2;
...
function approxTextBounds(label, textMetrics, rotated, rect) {
  ...
  var _padding$top2 = padding.top,
      top = _padding$top2 === void 0 ? PADDING : _padding$top2,
      _padding$bottom2 = padding.bottom,
      bottom = _padding$bottom2 === void 0 ? PADDING : _padding$bottom2,
      _padding$left2 = padding.left,
      left = _padding$left2 === void 0 ? PADDING : _padding$left2,
      _padding$right2 = padding.right,
      right = _padding$right2 === void 0 ? PADDING : _padding$right2;
...
function placeInBars(_ref3) {
  ...
  var placer = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : placeTextInRect$2;
  var postFilter = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : filterOverlappingLabels;
  var labels = [];
  ...
  for (var i = 0, len = targetNodes.length; i < len; i++) {
    ...
    for (var j = 0; j < target.texts.length; j++) {
      ...
      if (bounds && placement) {
        ...
        if (label) {
          var textBounds = approxTextBounds(label, measured, isRotated, bounds, placement.padding);
  ...
  return [].concat(_toConsumableArray(backgrounds), _toConsumableArray(filteredLabels));
}
...
function getHorizontalInsideSliceRect(_ref2) {
  ...
  var middle = normalize((start + end) / 2);
  var size = {
...
function sortNodes(nodes) {
  ...
  var sortFn = function sortFn(a, b) {
    var middleA = normalize((a.desc.slice.start + a.desc.slice.end) / 2);
    var middleB = normalize((b.desc.slice.start + b.desc.slice.end) / 2);
...
function slices(_ref10) {
  ...
  for (var i = 0, len = nodes.length; i < len; i++) {
    ...
    for (var j = 0; j < labelSettings.length; j++) {
      ...
      var linkData = typeof lblStngs.linkData === 'function' ? lblStngs.linkData(arg, i) : undefined;
      var measured = measureText$1(text, lblStngs, renderer);
      var bestPlacement = findPlacement({
        context: context,
        direction: direction,
        lblStngs: lblStngs,
...
function rows(_ref2) {
  ...
  defaults.fontSize = parseInt(defaults.fontSize, 10);
  var rowSettings = extend$1({}, defaults, settings);
  var labelSettings = settings.labels.map(function (labelSetting) {
    return extend$1({}, rowSettings, labelSetting);
  });
  var labelStruct = {};
  var labels = [];
  ...
  render: function render() {
    ...
    (stngs.sources || []).forEach(function (source) {
      if (source.strategy && strategies[source.strategy.type] && source.component) {
        labels.push.apply(labels, _toConsumableArray(strategy({
          chart: _this.chart,
          rect: _this.rect,
          renderer: _this.renderer,
          source: source,
...
function resolveSettings(comp) {
  ...
  if (comp.scale.type === 'threshold-color') {
    ...
    data.items = domain.map(function (d, idx) {
      var datum = comp.scale.datum ? extend$1({}, comp.scale.datum(d)) : {
        value: d
      };
      datum.value = d;
  ...
  var shapeSettings = extend$1(true, {}, (comp.settings.settings.item || {}).shape);
...
function createRenderItem(_ref) {
  ...
  var symbol = createSymbol(extend$1({}, symbolItem, wiggled));
...
function _itemize$2(_ref4, renderer) {
...
function _parallelize(availableExtent, availableSpread, itemized) {
  ...
  if (availableSpread != null) {
    ...
    var spreadMargin = spreadProperty === 'width' ? 'horizontal' : 'vertical';
    var spreadMarginSize = itemized.layout.margin[spreadMargin] || 4;
    var numAllowed = Math.floor((availableSpread + spreadMarginSize) / (spreadMarginSize + itemized.globalMetrics.maxItemBounds[spreadProperty]));
...
function itemRendererFactory (legend, _ref5) {
  ...
  var api = {
    itemize: function itemize(obj) {
      itemized = _itemize$2(obj, legend.renderer);
...
function _render$2(renderer, _ref3, itemized, legend) {
  var rect = _ref3.rect,
...
function navigationRendererFactory (legend) {
  ...
  var nav = {
    ...
    render: function render(obj) {
      return _render$2(nav.renderer, obj, itemized, legend);
    },
    extent: function extent() {
      return 32;
    },
...
function _itemize(_ref, legend) {
  ...
  var t = extend$1({}, resolved.title.item, {
    type: 'text'
  });
...
function categoricalLegend(picasso) {
  picasso.component(type$2, component$1);
}
...
function createTickNodes(ctx, legendNode) {
  ...
  return {
    type: 'container',
    id: 'legend-seq-ticks',
    children: [].concat(_toConsumableArray(nodes), [rangeSelectorRect])
  };
}
...
function initState(ctx) {
  ...
  var state = {
    ...
    ticks: {
      values: tickValues,
      anchor: tickAnchor,
      length: Math.min(Math.max.apply(Math, _toConsumableArray(tickValues.map(function (t) {
        return t.textMetrics.width;
      }))), ctx.stgns.tick.maxLengthPx),
      requiredHeight: function requiredHeight() {
        return tickAnchor === 'top' ? Math.max.apply(Math, _toConsumableArray(state.ticks.values.map(function (t) {
          return t.textMetrics.height;
        }))) + ctx.stgns.tick.padding : 0;
      },
      height: Math.max.apply(Math, _toConsumableArray(tickValues.map(function (t) {
        return t.textMetrics.height;
      })))
    },
    legend: {
...
var legendDef = {
  ...
  preferredSize: function preferredSize(opts) {
    ...
    if (state.ticks.anchor === 'left' || state.ticks.anchor === 'right') {
      ...
      prefSize += Math.max.apply(Math, _toConsumableArray(state.ticks.values.map(function (t) {
        return t.textMetrics.height;
      })));
    }
...
function line(picasso) {
  picasso.component(type$1, lineMarkerComponent);
}
...
function render$5(data, placement, _ref) {
...
var component = {
  ...
  init: function init(settings) {
    ...
    this.dispatcher.on(['cancelled', 'fulfilled'], function () {
      ...
      if (typeof _this2.props.beforeHide === 'function') {
        _this2.props.beforeHide.call(undefined, extend$1({
          element: _this2.state.tooltipElm
        }, listenerCtx));
      }

      if (typeof _this2.props.onHide === 'function') {
        _this2.props.onHide.call(undefined, extend$1({
          element: _this2.state.tooltipElm
        }, listenerCtx));
      } else {
  ...
  invokeRenderer: function invokeRenderer(nodes) {
    ...
    var pseudoElement = render$5(items, {
      style: {
        left: '0px',
        top: '0px',
        visibility: 'hidden'
    ...
    this.state.tooltipElm = render$5(items, pos, this);
  }
};
...
function toPoints$1(s, cp1, cp2, e) {
  ...
  var maxNbrOfSplits = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : 8;
...
function pathToPoints(path) {
  var commands = src.parsePath(path);
  var segments = [];
  var points = [];
  ...
  for (var i = 0; i < commands.length; ++i) {
    ...
    switch (pathType) {
      ...
      case 'a':
        points.push.apply(points, _toConsumableArray(arcToPoints(cmd, x, y)));
        x += cmd[6];
        y += cmd[7];
        break;
      ...
      case 'c':
        points.push.apply(points, _toConsumableArray(toPoints$1({
          x: x,
          y: y
        }, {
          x: cmd[1] + x,
      ...
      case 'Q':
        points.push.apply(points, _toConsumableArray(toPoints({
          x: x,
          y: y
        }, {
          x: cmd[1],
    ...
    get: function get() {
      if (!this._ancestors) {
        ...
        if (this.parent) {
          ...
          (_this$_ancestors = this._ancestors).push.apply(_this$_ancestors, [this.parent].concat(_toConsumableArray(this.parent.ancestors)));
        }
      }
    ...
    get: function get() {
      ...
      for (i = 0, len; i < len; i++) {
        ...
        if (c.children.length) {
          r.push.apply(r, _toConsumableArray(c.descendants));
        }
      }
...
var GeoRect = /*#__PURE__*/function () {
  ...
  _createClass(GeoRect, [{
    ...
    value: function intersectsGeoPolygon(geopolygon) {
...
var GeoCircle = /*#__PURE__*/function () {
  function GeoCircle() {
    var _ref = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {},
        _ref$cx = _ref.cx,
    ...
    _classCallCheck(this, GeoCircle);
...
var GeoPolyline = /*#__PURE__*/function () {
  ...
  _createClass(GeoPolyline, [{
    ...
    value: function intersectsGeoPolygon(geopolygon) {
      // This is a unoptimized solution and should be replaced by a more efficient algorithm.
      return this.segments.some(function (line) {
        return testGeoPolygonLine(geopolygon, line);
      });
    }
    /**
     * Get the points
...
reg$2.add('rect', create$k);
reg$2.add('circle', create$j);
reg$2.add('line', create$i);
reg$2.add('polygon', create$h);
reg$2.add('geopolygon', create$g);
reg$2.add('polyline', create$f);
/* eslint-disable import/prefer-default-export */

function create$e(type, input) {
...
var GeometryCollection = /*#__PURE__*/function () {
  ...
  _createClass(GeometryCollection, [{
    ...
    value: function set() {
      ...
      collection.forEach(function (geo) {
        var geoInstance = create$e(geo.type, geo);
    ...
    value: function intersectsGeoPolygon(geopolygon) {
      return this.geometries.some(function (geo) {
        return geo.intersectsGeoPolygon(geopolygon);
      });
    }
  }]);
...
function resolveMatrixCmd(matrix, transform) {
  if (transform.args.length >= 6) {
    matrix.transform.apply(matrix, _toConsumableArray(transform.args));
  }
}
...
function geometryToDef(geometry, dpi, mvm) {
  ...
  if (mvm) {
    if (points.every(function (item) {
      return Array.isArray(item);
    })) {
      points = points.map(function (item) {
        return mvm.transformPoints(item);
      });
    } else {
  ...
  if (type === 'rect' || type === 'bounds') {
    ...
    for (var i = 0; i < points.length; i++) {
      _path += pointsToPath(points[i], true);
    }
...
function inverseTransform(node, input) {
  ...
  if (Array.isArray(transformedInput.vertices)) {
    if (transformedInput.vertices.every(function (item) {
      return Array.isArray(item);
    })) {
      transformedInput = create$g(transformedInput);
    } else {
      transformedInput = create$h(transformedInput); // TODO Shouldn't have to do this here, currently its beacause a collision algorithm optimization, i.e. caching of polygon bounds
    }
  }
...
function resolveShape(shape) {
  ...
  switch (type) {
    ...
    case 'geopolygon':
      _shape.vertices = shape.vertices.map(function (vertices) {
...
var DisplayObject = /*#__PURE__*/function (_Node) {
  ...
  _createClass(DisplayObject, [{
    ...
    get: function get() {
      ...
      switch (this._collider.type) {
        case 'collection':
          this._collider.fn = create$d(this._collider.definition);
          break;
        ...
        case 'bounds':
          this._collider.fn = create$e('rect', this.boundingRect());
          break;
        ...
        case 'geopolygon':
        case 'polyline':
          this._collider.fn = create$e(this._collider.type, this._collider.definition);
          break;
...
var NodeContainer = /*#__PURE__*/function (_Node) {
  _inherits(NodeContainer, _Node);

  var _super = _createSuper(NodeContainer);
  ...
  _createClass(Container, [{
    ...
    value: function removeChild(c) {
      ...
      if (this._collider && this._collider.type === 'bounds') {
        ...
        var opts = extend$1(this.boundingRect(true), {
          type: 'bounds'
        });
        this.collider = opts;
      }
...
var allowedAttrs$1 = ['x1', 'x2', 'y1', 'y2', 'id', 'offset', 'style'];

var GradientItem = /*#__PURE__*/function (_DisplayObject) {
  _inherits(GradientItem, _DisplayObject);

  var _super = _createSuper(GradientItem);
  ...
  function GradientItem() {
    ...
    _classCallCheck(this, GradientItem);
...
var allowedAttrs = ['patternUnits', 'x', 'y', 'width', 'height', 'id'];

var PatternItem = /*#__PURE__*/function (_DisplayObject) {
  _inherits(PatternItem, _DisplayObject);

  var _super = _createSuper(PatternItem);
  ...
  function PatternItem() {
    ...
    _classCallCheck(this, PatternItem);
  ...
  _createClass(Path, [{
    ...
    value: function set() {
      ...
      if (Array.isArray(v.collider) || _typeof(v.collider) === 'object' && typeof v.collider.type !== 'undefined') {
        ...
        if (this.segments.length > 1 && this.segments.every(function (segment) {
          return isClosed(segment);
        })) {
...
reg$1.add('rect', create$6);
reg$1.add('circle', create$5);
reg$1.add('text', create$2);
reg$1.add('line', create$4);
reg$1.add('path', create$3);
reg$1.add('stage', create$9);
reg$1.add('container', create$a);
reg$1.add('defs', create$a);
reg$1.add('linearGradient', create$8);
reg$1.add('radialGradient', create$8);
reg$1.add('stop', create$8);
reg$1.add('pattern', create$7);
/* eslint-disable import/prefer-default-export */
...
function traverse(items, parent, matrix, on) {
  ...
  for (var i = 0, len = items.length; i < len; i++) {
    ...
    var displayNode = create$1(state.node.type, state.node);
...
function render$1(t, _ref) {
  var g = _ref.g,
      ellipsed = _ref.ellipsed;
  var text = ellipsed || ellipsText(t, measureText);
...
register('circle', render$3);
register('line', render$2);
register('path', render);
register('text', render$1);
...
function diff(from, to) {
  ...
  for (var _i2 = 0, _len2 = added.length; _i2 < _len2; _i2++) {
    if (added[_i2].content.children) {
      added[_i2].diff = diff([], added[_i2].content.children);
  ...
  for (var _i3 = 0, _len3 = updatedNew.length; _i3 < _len3; _i3++) {
    updatedNew[_i3].diff = diff(updatedOld[_i3].children || [], updatedNew[_i3].content.children || []);
...
var svgNs = 'http://www.w3.org/2000/svg'; // const badValues = ['', NaN, 'NaN', undefined, null, false, true];
...
var TreeItemRenderer = /*#__PURE__*/function () {
  /**
   * Constructor
   * @private
  ...
  function TreeItemRenderer(treeCreator, nodeCreator, nodeMaintainer, nodeDestroyer) {
    _classCallCheck(this, TreeItemRenderer);
...
function renderer$1() {
  ...
  var patterns = patternizer(defs.children);
  ...
  svg.render = function (nodes) {
    ...
    var sceneContainer = {
      type: 'container',
      children: Array.isArray(nodes) ? [].concat(_toConsumableArray(nodes), [defs]) : nodes,
...
function renderer() {
  ...
  dom.render = function (nodes) {
    ...
    if (Array.isArray(nodes)) {
      vNode = a("div", null, nodes);
    } else {
      vNode = nodes;
    }

    dNode = N(vNode, el, dNode);
    return true;
  };

  dom.renderArgs = [a]; // Arguments to render functions using the DOM renderer
